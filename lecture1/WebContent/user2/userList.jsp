...
<%@ page import="java.util.*, java.net.*, lecture1.user2.*, lecture1.*" %>
...
<a id="createButton" class="btn btn-primary pull-right" 
   href="userCreate1.jsp?pg=<%=currentPage%>&srchText=<%=srchTextEncoded%>&od<%=od%>">
...
<form class="form-inline">
  <div class="form-group">
  	...
  	<select name="od" class="form-control">
  		...
  		<option value="2" <%= "2".equals(od) ? "selected" : "" %>>사용자유형</option>
...
<table class="table table-bordered table-condensed">
  ...
  <tbody>
    <% for (User user : list) { %>
      <tr data-url="userEdit1.jsp?id=<%=user.getId()%>&pg=<%=currentPage%>&srchText=<%=srchTextEncoded%>&od=<%=od%>">
        <td><%= user.getId() %></td>
